...
(0, utils_1.addCliParams)((0, config_1.task)("trace", "Traces a transaction hash"))
    ...
    if (tx && hre.network.name === "hardhat") {
        ...
        if (lastTrace === undefined) {
            throw new Error("[hardhat-tracer]: No trace available on current provider");
        }
        await (0, print_1.print)(lastTrace, {
    ...
    if (hre.network.name !== "hardhat") {
        throw new Error("[hardhat-tracer]: Please use --rpc flag and do not pass a --network flag for this one");
    }
    ...
    if (trace === undefined) {
        throw new Error("[hardhat-tracer]: No trace available on the mainnet forked provider");
    }
    debug("printing trace");
    await (0, print_1.print)(trace, {
        artifacts: hre.artifacts,
        tracerEnv: hre.tracer,
        provider: hre.ethers.provider,
    });
