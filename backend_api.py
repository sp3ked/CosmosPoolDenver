...
def get_all_balances(contract, pool_address):
    balances = {
        ...
        "Depositor_user_weth": {
            "WETH": get_token_balance(WETH_ADDRESS, user_weth)
        },
        "Depositor_user_usdc": {
            "USDC": get_token_balance(USDC_ADDRESS, user_usdc)
        },
        "Owner": owner
    }
    return balances
...
def deposit():
    ...
    if token_type not in ["WETH", "USDC"] or amount is None:
        return jsonify({"error": "Invalid parameters. Require token_type and amount."}), 400
    try:
